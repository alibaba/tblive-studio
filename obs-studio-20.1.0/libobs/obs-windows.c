...
static int has_set_cpu_name = 0;
static wchar_t cpu_name[1024];
static int has_set_windows_ver = 0;
static char windows_ver[128];
...
void query_physical_memory_state(const char* memory_info) {
	if (memory_info) {
		MEMORYSTATUSEX ms;
		ms.dwLength = sizeof(ms);
		GlobalMemoryStatusEx(&ms);
		snprintf(memory_info, 128, "Physical Memory: %luMB Total, %luMB Free",
			(DWORD)(ms.ullTotalPhys / 1048576),
			(DWORD)(ms.ullAvailPhys / 1048576));
	}
}

void query_system_state(wchar_t* cpu_info, char* system_info) {
	if (cpu_info && has_set_cpu_name) {
		memcpy(cpu_info, cpu_name, 1024);
	}
	
	if (system_info && has_set_windows_ver) {
		memcpy(system_info, windows_ver, 128);
	}
}
...
static void log_windows_version(void)
{
	...
	memset(windows_ver, 0, 128);

	snprintf(windows_ver, sizeof(windows_ver), "Windows Version: %d.%d Build %d (revision: %d; %s-bit)",
		ver.major, ver.minor, ver.build, ver.revis, windows_bitness);

	has_set_windows_ver = 1;

	blog(LOG_INFO, "%s", windows_ver);
}

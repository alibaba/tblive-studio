...
#include "obs-scene.h"
#include "source-type-defines.hpp"
...
obs_sceneitem_t *obs_sceneitem_from_source(obs_scene_t *scene,obs_source_t *source)
{
    struct obs_scene_item *item;
...
bool obs_scene_exist_source_with_id(obs_scene_t *scene, const char *id)
{
	struct obs_scene_item *item;
	...
	item = scene->first_item;
	while (item) {
		if (strcmp(item->source->info.id, id) == 0) {
			full_unlock(scene);
			return true;
		}
...
bool obs_scene_video_device_used(obs_scene_t *scene, obs_data_t* data, const char *device_str)
{
    ...
    item = scene->first_item;
    while (item) {
        if (strcmp(item->source->info.id, SOURCE_DSHOW_INPUT) == 0
            && item->source->async_active) {
            settings = obs_source_get_settings(item->source);
            ...
            name = obs_data_get_string(settings, VIDEO_DEVICE_ID);

            if (strcmp(name, device_str) == 0) {
                used = true;
                obs_data_release(settings);
                break;
            }
...
bool obs_scene_has_free_capture_device(obs_scene_t *scene)
{
    obs_sceneitem_t *item = NULL;
    ...
    while (item) {
        if (strcmp(item->source->info.id, SOURCE_DSHOW_INPUT) == 0
            ...
            blog(LOG_INFO, "camera source %p, active %d", source, source->active);
        }
        item = item->next;
    }
    ...
    while (property) {
        ...
        if (strcmp(property_name, VIDEO_DEVICE_ID) == 0) {
            device_count = obs_property_list_item_count(property);
            break;
        }
    ...
    blog(LOG_INFO, "camera device num: %d, camera source num: %d",
        device_count, source_count);

    return (device_count > source_count);
}
...
bool obs_scene_items_with_id(obs_sceneitem_t* item, const char* id) {

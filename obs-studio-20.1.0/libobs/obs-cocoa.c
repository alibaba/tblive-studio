...
void query_system_state(char* cpu_info, char* system_info) {
    ...
    if (system_info) {
        memcpy(system_info, osx_ver, 128);
    }
}
...
void query_physical_memory_state(char* memory_info) {
    ...
    if (ret == 0) {
        snprintf(memory_info, 128, "Physical Memory: %lldMB Total",
...
static void log_os_version(id pi, SEL UTF8String)
{
    ...
    snprintf(osx_ver, 128, "OSX Version: %s", version ? version : "Unknown");
}

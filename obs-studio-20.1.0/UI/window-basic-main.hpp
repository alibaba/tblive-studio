...
private:
	...
	//QPointer<ElementsDialog> elementsDialog;
    QPointer<TBLiveElementsView> elementView;

	// 埋点相关 --------------start---------------
	QPointer<QTimer>    monitorInfoUploadTimer;
	int64_t             monitorInfoUploadCount = 0;

    std::atomic<int64_t> totalStreamSeconds = {0};
    std::atomic<double> outputBandwidth = {0.0};
#if defined(_WIN32)
	std::string         platform = "Win";
#else
	std::string         platform = "Mac";
#endif
	//std::string         tblsVersion = tblive_get_version_string();
	std::map<std::string, int16_t> currentVideoCaptureFPS;
	// 埋点相关 --------------end-----------------

	//推流详情窗口
	QPointer<StreamStateWindow> stateWindow;
	QPointer<QTimer>    stateWindowUpdateTimer;

	// 摄像头名称集
	std::map<std::wstring, bool> cameras;

	OBSService service;
	std::unique_ptr<BasicOutputHandler> outputHandler;
    std::unique_ptr<VolHoverWidget> m_pHoverVolCtrlWidget;
    std::unique_ptr<ResolutionDockWidget> m_pReslSelWidget;
    std::unique_ptr<OBSBasicEnd> m_pWindEnd;
    ...
    void InitResolutionSelectWidget();
    void StartCustomStreaming();
    void StopCustomStreaming();
    void SetStreamURL(const char* url);
    void AddSourceWithProperty(const char *id);
    void UpdateNewSource(obs_source_t * source,
        const char *id, const char *url);
    const char* GetSourceNameWithID(const char *id);
...
private slots:
    ...
    void RemoveItemFromSource(OBSSource source);

    bool AddSourcewithURL(const char *id, const char *url);
    bool AddElementSource(const char *id, const char* name, const char *url);
	...
	// 埋点相关 --------------start---------------
	void UploadMonitorInfo(); // add by WeiHe
	inline void UpdataStreamTime() { totalStreamSeconds++; }
	inline void UpdateOutputBandwidth(double bandwidth){ outputBandwidth = bandwidth; }
	void GetCurrentVideoCaptureFPS();
	void UploadStreamTimeDuration();
	// 埋点相关 ---------------end---------------
    
    void UploadStreamInfo();
	void CheckIfNewVideoDeviceAdded();

	void updateStateWindow();

	void ResetElementImageSourcePosition(OBSSource source);
...
private:
    ...
    static void SourceRemoveCallback(void *data, calldata_t *params);
	static void SourceStartRendering(void* data, calldata_t* params);
    ...
    void InitTbliveUI();
    void UploadTBLiveLog(const char *file);
    void UploadSelectedLog();

	void SetupStreamStateWindow();

	void ConfigSourceIfNeeded(obs_source_t * source, const char* source_url);
	void CloseElementDialog();
    void ConfigVideoEncoderParam();
...
public:
    ...
    void OnAudioMixerBtn();
    void SetStreamParams();
    void UpdataResolutionIcon();
    void SetCustomBasicConfig(bool is_720p);
...
private slots:
    ...
    void on_actionUploadSelectedLog_triggered();
	void on_actionViewCurrentLog_triggered();
	void on_actionCheckForUpdates_triggered();

	void on_actionShowStateWindow_triggered();
    ...
    void on_selectResolution_clicked();
    void on_enterLivePlatformBtn_clicked();
    void on_beautifyBtn_clicked();
    void on_elementsBtn_clicked();

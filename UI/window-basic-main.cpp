...
static void AddExtraModulePaths()
{
	...
	BPtr<char> config_bin = os_get_config_path_ptr("TBLiveStudio/plugins/%module%/bin");
	BPtr<char> config_data = os_get_config_path_ptr("TBLiveStudio/plugins/%module%/data");
...
void OBSBasic::InitOBSCallbacks()
{
    ...
    signalHandlers.emplace_back(obs_get_signal_handler(), "source_remove_callback",
        OBSBasic::SourceRemoveCallback, this);
	signalHandlers.emplace_back(obs_get_signal_handler(), "source_start_rendering",
		OBSBasic::SourceStartRendering, this);
}
...
void OBSBasic::OBSInit()
{
	...
	ret = snprintf(fileName, 512, "TBLiveStudio/basic/scenes/%s.json",
			sceneCollection);
	if (ret <= 0)
...
#undef SET_VISIBILITY
    ...
    //AddVideoCapture();
    AddSourcewithURL(SOURCE_DSHOW_INPUT, nullptr);
}
...
OBSBasic::~OBSBasic()
{
	...
	UploadStreamTimeDuration();

	if (monitorInfoUploadTimer) {
		delete monitorInfoUploadTimer;
	}

	if (stateWindowUpdateTimer) {
		delete stateWindowUpdateTimer;
	}
...
void OBSBasic::ActivateAudioSource(OBSSource source)
{
    ...
    m_pHoverVolCtrlWidget->AddWidget2Layout(vol);
}
...
int OBSBasic::ResetVideo()
{
    ...
    ovi.face_beauty_enable = m_faceBeautyEnabled;
	ovi.face_beauty_time = 0;



    blog(LOG_INFO, "ResetVideo: base(%dx%d), output(%dx%d)",
         ovi.base_width, ovi.base_height, ovi.output_width, ovi.output_height);

	if (ovi.base_width == 0 || ovi.base_height == 0) {
		ovi.base_width = 1280;
		ovi.base_height = 720;
		config_set_uint(basicConfig, "Video", "BaseCX", 1280);
		config_set_uint(basicConfig, "Video", "BaseCY", 720);
	}
...
void OBSBasic::on_actionUploadCurrentLog_triggered()
{
    ...
    UploadTBLiveLog(App()->GetCurrentLog());
}
...
void OBSBasic::on_actionUploadSelectedLog_triggered()
{
    UploadSelectedLog();
}
...
void OBSBasic::on_actionShowStateWindow_triggered() {
...
//    ui->startStreamBtn->setStyleSheet("QPushButton {background-image:url(:/res/icons/start-button-disable.png);}"
//                                      "QPushButton {background-position: center; background-repeat: no-repeat;}");
...
//        ui->startStreamBtn->setStyleSheet("QPushButton {background-image:url(:/res/icons/start-button.png);}"
//                                          "QPushButton {background-position: center; background-repeat: no-repeat;}");
...
inline void OBSBasic::OnActivate()
{
	if (ui->profileMenu->isEnabled()) {
		...
		if (trayIcon)
			trayIcon->setIcon(QIcon(":/res/icons/icon-live.png"));
	}
}
...
//    ui->stopStreamBtn->setStyleSheet("QPushButton {background-image:url(:/res/icons/end-button.png);}"
//                                      "QPushButton {background-position: center; background-repeat: no-repeat;}");
	...
	// 埋点信息上传 add by WeiHe
	if (!monitorInfoUploadTimer) {
		monitorInfoUploadTimer = new QTimer(this);
		connect(monitorInfoUploadTimer, SIGNAL(timeout()), this, SLOT(UploadMonitorInfo()));
		monitorInfoUploadTimer->start(1000);
	}
	else if (!monitorInfoUploadTimer->isActive()){
		monitorInfoUploadTimer->start(1000);
	}
...
//    ui->stopStreamBtn->setStyleSheet("QPushButton {background-image:url(:/res/icons/end-button-disable.png);}"
//                                     "QPushButton {background-position: center; background-repeat: no-repeat;}");
...
void OBSBasic::StreamingStop(int code, QString last_error)
{
	...
	// 
	UploadStreamTimeDuration();
	if (monitorInfoUploadTimer && monitorInfoUploadTimer->isActive()) {
		monitorInfoUploadTimer->stop();
	}
...
{
	char path[512];
	int ret = GetConfigPath(path, 512, "TBLiveStudio");
	if (ret <= 0)
		return;
...
//            "Basic", "SceneCollection");
	...
	setWindowTitle(QT_UTF8(name.str().c_str()).append(QString(TBLiveEngine::Instance()->GetVerString())));
}
...
int OBSBasic::GetProfilePath(char *path, size_t size, const char *file) const
{
	...
	ret = GetConfigPath(profiles_path, 512, "TBLiveStudio/basic/profiles");
	if (ret <= 0)
		return ret;
...
{
	trayIcon = new QSystemTrayIcon(QIcon(":/res/icons/icon.png"),
			this);
	trayIcon->setToolTip("TBLiveStudio");
		...
		QSystemTrayIcon::MessageIcon n)
{
	if (QSystemTrayIcon::supportsMessages()) {
		QSystemTrayIcon::MessageIcon icon =
				QSystemTrayIcon::MessageIcon(n);
		trayIcon->showMessage((QTStr("TitleBar.Title")), text, icon, 10000);
	}
}
...
void OBSBasic::ResetElementImageSourcePosition(OBSSource source) {
	auto func = [](obs_scene_t *scene, obs_sceneitem_t *item, void *param)
	{
		...
		vec2 item_pos, item_offset;
		vec2_set(&item_pos, float(ovi.base_width),
			float(ovi.base_height));
		vec2_mulf(&item_pos, &item_pos, 0.5f);

		int width = obs_source_get_base_width(source);
		int height = obs_source_get_base_height(source);
		vec2_set(&item_offset, float(width / 2.0), float(height / 2.0));

		vec2_sub(&item_pos, &item_pos, &item_offset);
		obs_sceneitem_set_pos(item, &item_pos);

		return true;
	};
	obs_scene_enum_items(GetCurrentScene(), func, source);
}

		...
		obs_data_t *settings, obs_data_t *hotkey_data)
{
	...
	pthread_mutex_init_value(&output->congest_mutex);
	...
	if (pthread_mutex_init(&output->caption_mutex, NULL) != 0)
		goto fail;
	if (pthread_mutex_init(&output->congest_mutex, NULL) != 0)
		goto fail;
...
void obs_output_destroy(obs_output_t *output)
{
	if (output) {
		...
		pthread_mutex_destroy(&output->congest_mutex);
...
bool obs_output_get_stream_congest(obs_output_t *output) {
	if (!obs_output_valid(output, "obs_output_stream_congest")) {
		return false;
	}

	pthread_mutex_lock(&output->congest_mutex);
	bool congest = output->stream_congest;
	output->stream_congest = false;
	pthread_mutex_unlock(&output->congest_mutex);
	return congest;
}

void obs_output_set_stream_congest(obs_output_t *output, bool congest) {
	pthread_mutex_lock(&output->congest_mutex);
	output->stream_congest = congest;
	pthread_mutex_unlock(&output->congest_mutex);
}

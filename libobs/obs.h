...
typedef struct obs_system_info {
#if defined(_WIN32)
	wchar_t cpu_name[1024];
#else
	char cpu_name[1024];
#endif
...
struct obs_video_info {
	...
	bool                gpu_conversion;
	bool 				face_beauty_enable;
	uint32_t            face_beauty_time;  // time usage in ms per image 
...
EXPORT void obs_query_system_info(obs_system_info_t* system_info);
...
EXPORT int obs_reset_face_beauty_enable(bool enable);
EXPORT int64_t obs_get_face_beauty_time();
...
/** Gets the current camera name */
EXPORT const wchar_t* obs_get_camera_name(struct obs_source* source);
...
EXPORT uint32_t obs_get_video_encode_fps(void);
//EXPORT uint32_t tbls_get_audio_encode_fps(void);
...
EXPORT void test(obs_source_t *source);
...
EXPORT obs_sceneitem_t *obs_sceneitem_from_source(obs_scene_t *scene,
    obs_source_t *source);

EXPORT bool obs_scene_exist_source_with_id(obs_scene_t *scene,
        const char *id);

EXPORT void obs_source_remove_callback(obs_source_t *source);
EXPORT void obs_source_start_rendering(obs_source_t *source);

EXPORT bool obs_scene_video_device_used(obs_scene_t *scene,
    obs_data_t* data, const char *device_name);

EXPORT bool obs_scene_has_free_capture_device(obs_scene_t *scene);
...
EXPORT bool obs_scene_items_with_id(obs_sceneitem_t* item, const char* id);
...
// 
EXPORT bool obs_output_get_stream_congest(obs_output_t *output);
EXPORT void obs_output_set_stream_congest(obs_output_t *output, bool congest);

...
static const char *obs_signals[] = {
    ...
    "void source_remove_callback(ptr source)",
	"void source_start_rendering(ptr source)",
...
#if defined(_WIN32)
extern void query_system_state(wchar_t* cpu_info, char* system_info);
#else
extern void query_system_state(char* cpu_info, char* system_info);
#endif
extern void query_physical_memory_state(char* memory_info);
void obs_query_system_info(obs_system_info_t* system_info) {
	query_system_state(system_info->cpu_name, system_info->system_name);
	query_physical_memory_state(system_info->memory_info);
}
...
int obs_reset_face_beauty_enable(bool enable)
{
	if(!obs) return OBS_VIDEO_FAIL;

	obs->video.ovi.face_beauty_enable = enable;

	return OBS_VIDEO_SUCCESS;
}

int64_t obs_get_face_beauty_time() {
	if (!obs) return OBS_VIDEO_FAIL;
	
	return os_atomic_load_long(&obs->video.ovi.face_beauty_time);
}
...
{
    pthread_mutex_lock(&obs->data.sources_mutex);
    blog(LOG_INFO, "start to free context data: %s", context->name);

...
static void image_source_load(struct image_source *context)
{
	...
	if (file && *file) {
		...
		if (!context->image.loaded) {
			...
			obs_source_start_rendering(context->source);
		}
	}
}
...
static void image_source_update(void *data, obs_data_t *settings)
{
    ...
    context->image.enable_loop = obs_data_get_bool(settings, "looping");
    context->image.element_type = obs_data_get_int(settings, "element_type");
}
...
static void image_source_defaults(obs_data_t *settings)
{
	...
	obs_data_set_default_int(settings, "element_type", -1);
}
...
static void image_source_render(void *data, gs_effect_t *effect)
{
    ...
    //blog(LOG_INFO, "render frame: %d", context->image.cur_frame);
...
static void image_source_tick(void *data, float seconds)
{
	...
	if (context->last_time && context->image.is_animated) {
        if (context->image.play_finished && !context->image.enable_loop){
            obs_source_remove_callback(context->source);
            return;
        }

...
#define g_camera_name_buffer_size 128
static wchar_t g_camera_name_bufer[g_camera_name_buffer_size];

static const wchar_t* GetDShowVideoDeviceName(void *data) {
	DShowInput *input = reinterpret_cast<DShowInput*>(data);
	if (input && input->videoConfig.name.size() < g_camera_name_buffer_size) {
		memset(g_camera_name_bufer, 0, g_camera_name_buffer_size);
		memcpy(g_camera_name_bufer, input->videoConfig.name.c_str(), input->videoConfig.name.size() * sizeof(wchar_t));
		return g_camera_name_bufer;
	}
	else {
		return NULL;
	}
...
static void GetDShowDefaults(obs_data_t *settings)
{
	...
	obs_data_set_default_bool(settings, USE_CUSTOM_AUDIO, false);
...
void RegisterDShowSource()
{
	...
	info.get_device_name = GetDShowVideoDeviceName;
